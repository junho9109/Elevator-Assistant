...
// PPE routes
  ...
  // TEMP DEBUG: 팀별 10개 항목 재구성 - 중복 배치(id 48~78) 정리 + 팀2/3/4 부족분 보강 (사용 후 제거 예정)
  app.get("/api/debug/fix-team-item-quotas", async (req, res) => {
    try {
      if (req.query.secret !== "elev2026fix") return res.status(403).json({ error: "forbidden" });
      const { pool: pgPool } = await import("./db");
      // 1) 중복 배치(id 48~78)를 실제로 선택/평가한 사용자가 있는지 먼저 확인 — 있으면 삭제 중단
      const selRows = await pgPool.query(
        `SELECT hazard_item_id FROM risk_item_selections WHERE hazard_item_id BETWEEN 48 AND 78`
      );
      const asmRows = await pgPool.query(
        `SELECT hazard_item_id FROM risk_assessments WHERE hazard_item_id BETWEEN 48 AND 78`
      );
      if (selRows.rows.length > 0 || asmRows.rows.length > 0) {
        return res.status(409).json({
          error: "in_use",
          detail: "중복 배치(id 48~78) 항목에 이미 선택/평가 데이터가 존재하여 삭제를 중단했습니다.",
          selections: selRows.rows,
          assessments: asmRows.rows,
        });
      }
      // 2) 중복 배치 삭제
      const del = await pgPool.query(`DELETE FROM risk_hazard_items WHERE id BETWEEN 48 AND 78`);
      // 3) 팀2(유압식 엘리베이터)/팀3(에스컬레이터 및 휠체어리프트)/팀4(사무업무) 부족분 보강
      const items = [
        // 팀2 - 유압식 엘리베이터 2반 (기존 8개 + 필수1 → 10개로 보강)
        {
          method: "freq_severity", workCategory: "엘리베이터", subWork: "카내(승강장) 검사",
          content: "문열림출발방지장치 검사 후 끼임 등으로 잘 닫히지 않는 승강장문을 닫는 과정에서 손이 끼이는 위험",
          team: "유압식 엘리베이터 2반",
          referenceSafetyMeasure: "승강장문을 무리한 힘을 가해서 닫는 행위는 자제하고, 필요 시 유지관리업체에게 조치를 요청한다. 문이 한번에 완전히 닫히지 않도록 하고, 닫는 행위 전에 지지대 등을 이용해 끼임을 방지한다.",
        },
        {
          method: "freq_severity", workCategory: "엘리베이터", subWork: "카상부 검사",
          content: "화물용 승강기 카상부 이동 중 천장에 설치된 조명을 밟아 추락하는 위험(아차사고사례)",
          team: "유압식 엘리베이터 2반",
          referenceSafetyMeasure: "카 천장의 조명 등 충분한 강도가 확보되지 않은 구조물은 밟지 않도록 주의하고, 보강 조치하도록 관리주체에게 권고한다.(종전 검사기준 적용 현장)",
        },
        // 팀3 - 에스컬레이터 및 휠체어리프트 3반 (기존 8개 → 10개로 보강)
        {
          method: "freq_severity", workCategory: "에스컬레이터", subWork: "기계실",
          content: "뜨거운 구동기, 부품, 조명 등에 의한 화상 위험",
          team: "에스컬레이터 및 휠체어리프트 3반",
          referenceSafetyMeasure: "구동기, 부품, 조명 등이 식은 후 검사한다. 장갑, 토시 등을 착용한다.",
        },
        {
          method: "freq_severity", workCategory: "에스컬레이터", subWork: "기계실",
          content: "검사 준비 등으로 상부 또는 하부 플레이트가 열린 상태를 인지하지 못하고 이동 중 떨어지는 위험(산업재해 사례)",
          team: "에스컬레이터 및 휠체어리프트 3반",
          referenceSafetyMeasure: "이동 중 스마트단말기 사용을 자제하고, 검사 시작 전 유해·위험요인을 사전에 확인한다.",
        },
        // 팀4 - 사무업무 4반 (기존 4개 → 10개로 보강, 붙임4-1 체크리스트 원문 반영)
        {
          method: "checklist", workCategory: "사무", subWork: "사무기기 사용",
          content: "사무기구(세단기 등) 사용 시 손가락 끼임 등의 사고를 예방하기 위하여 위험경고 표시가 부착되어 있는가?",
          team: "사무업무 4반",
          referenceSafetyMeasure: "세단기 등 사무기구에 손가락 끼임 주의 경고 표시를 부착하고, 사용 전 안전장치 작동 여부를 확인한다.",
        },
        {
          method: "checklist", workCategory: "사무", subWork: "사무실 내 이동",
          content: "사무실 바닥에 노출된 전선에 걸려 넘어지지 않도록 보호조치가 되어 있는가?",
          team: "사무업무 4반",
          referenceSafetyMeasure: "바닥에 노출된 전선은 전선 커버 또는 배선 정리를 통해 걸림 위험을 제거한다.",
        },
        {
          method: "checklist", workCategory: "비상상황(화재·재난 등)", subWork: "소화·비상대응",
          content: "소화기 충전압력이 양호하도록 주기적으로 점검하고 있는가?",
          team: "사무업무 4반",
          referenceSafetyMeasure: "소화기 압력계를 월 1회 이상 점검하고, 불량 소화기는 즉시 교체한다.",
        },
        {
          method: "checklist", workCategory: "비상상황(화재·재난 등)", subWork: "소화·비상대응",
          content: "화재 등 비상 상황에서 비상계단이나 창문 등을 통해 대피할 수 있는가?",
          team: "사무업무 4반",
          referenceSafetyMeasure: "비상계단·비상구 앞에 적재물을 두지 않고, 대피 동선을 정기적으로 확인·공유한다.",
        },
        {
          method: "checklist", workCategory: "사무", subWork: "사무환경",
          content: "장시간 에어컨 및 히터로 인한 고온 및 저온 환경에 노출될 위험이 있는가?",
          team: "사무업무 4반",
          referenceSafetyMeasure: "냉난방기 설정온도를 적정 범위로 유지하고, 환기를 주기적으로 실시한다.",
        },
        {
          method: "checklist", workCategory: "사무", subWork: "사무기기 사용",
          content: "장시간 컴퓨터 사용으로 인한 VDT증후군 발생 위험을 예방하기 위하여 올바른 작업자세에 대한 교육을 실시하고 있는가?",
          team: "사무업무 4반",
          referenceSafetyMeasure: "1시간 사용 후 10분 휴식을 권장하고, 모니터 높이·의자 등 올바른 작업자세 교육을 정기적으로 실시한다.",
        },
      ];
      let inserted = 0;
      for (const it of items) {
        await pgPool.query(
          `INSERT INTO risk_hazard_items (method, work_category, sub_work, content, discovery_path, branch_id, registered_by_id, registered_by_name, team, is_template, is_mandatory)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,true,false)`,
          [it.method, it.workCategory, it.subWork, it.content, "붙임4-1/4-3 (2026년도 정기 위험성평가 서식)", "서울강서지사", "노준호", "노준호", it.team]
        );
        inserted++;
      }
      // referenceSafetyMeasure는 별도 컬럼 업데이트 (컬럼명 charset 이슈 방지 위해 content로 매칭)
      for (const it of items) {
        await pgPool.query(
          `UPDATE risk_hazard_items SET reference_safety_measure=$1 WHERE content=$2 AND team=$3 AND reference_safety_measure IS NULL`,
          [it.referenceSafetyMeasure, it.content, it.team]
        );
      }
      res.json({ ok: true, deletedDuplicateBatch: del.rowCount, inserted });
    } catch (error) {
