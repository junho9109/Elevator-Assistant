...
// 이하 SECTIONS / ITEMS 배열은 이전 스크립트와 동일
const SECTIONS = [
...
async function main() {
  ...
  console.log("📋 시작 - equipment_types: 한국어 표준 포맷");
  try {
    ...
    for (const sec of SECTIONS) {
      ...
      console.log(`  ✓ section: ${sec.item_id}`);
    }
    ...
    console.log(`\n✅ COMMIT 완료`);
    console.log(`  섹션 헤더: ${SECTIONS.length}개`);
    console.log(`  검사 항목: ${ITEMS.length}개`);
    console.log(`  🔍 검증: inspection_item_edits 신규 항목 수 = ${check.rows[0].count}`);

    if (check.rows[0].count === "0") {
      console.error("❌ 검증 실패: COMMIT 후에도 0개. 트랜잭션 격리/RLS 의심.");
    }
  } catch (err) {
    await client.query("ROLLBACK");
    console.error("\n❌ ROLLBACK 발생:", err.message);
